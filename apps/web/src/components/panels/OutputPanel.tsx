    ...
    <div className="flex flex-col min-h-72 h-full border border-[var(--color-border)] rounded-md bg-[var(--color-surface)] overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--color-border-muted)]">
        ...
        <button
          onClick={handleCopy}
          disabled={!output}
          className="flex items-center gap-1 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-150"
        >
          {copied ? (
      ...
      <div className={`flex-1 p-3 overflow-auto ${isPending ? 'animate-pulse opacity-70' : ''}`}>
        {output ? (
          <HighlightedCode code={output} lang={lang} />
        ) : (
          <span className="text-[var(--color-text-disabled)] text-[13px] font-mono">
